...
export default function ClientPage() {
    ...
    useEffect(() => {
        if (currentUser) {
            axios.get(HOST.device_backend_api + "/device/getByUser/" + currentUser.id)
                .then((response) => {
    ...
    if (!currentUser) {
        return (
            <div>Invalid login. Redirecting to the login page...</div>
        );
    }
                ...
                <ListGroup style={{listStyle: 'none', padding: '0'}}>
                    {userDevices.map((device) => (
                        <ListGroupItem key={device.id}>
                            <h3 style={{marginBottom: '10px'}}>{device.description}</h3>
                            <p><strong>Address:</strong> {device.address}</p>
                            <p><strong>Max Consumption:</strong> {device.maxConsumption}</p>
                            <hr style={hrStyle}/>
                        </ListGroupItem>
                    ))}
                </ListGroup>
